...
class FixedGPUBacktestSystem:
    ...
    def __init__(self, initial_capital=INITIAL_CAPITAL, max_positions=MAX_POSITIONS, 
        ...
        print(f"🔧 修复版系统配置: 使用{'GPU' if self.use_gpu else 'CPU'}加速, {self.num_workers}个工作进程")
        print(f"📝 策略条件: 放宽版 (weekly_mean_down 或 简化条件)")
    ...
    def _get_strategy_recommendations(self, date):
        ...
        for code in test_codes:
            try:
                signal = self._calculate_stock_signals_relaxed(code)
                if signal:
        ...
        if not df.empty:
            # 使用综合得分排序
            df['score'] = df.apply(self._calculate_stock_score_relaxed, axis=1)
            df = df.sort_values('score', ascending=False)
        
        print(f"  ✅ 策略推荐: {len(df)} 只")
        return df.head(self.max_positions * 2)  # 返回两倍于最大持仓的数量，供选择
    
    def _calculate_stock_signals_relaxed(self, stock_code):
        """计算单只股票的策略信号（放宽版）"""
        try:
            # 检查ST
            ...
            # 获取日线数据
            daily_data = self.base_op.get_daily_data(stock_code, days=20)  # 减少到20天
            ...
            # 放宽条件1：不严格要求cross_ma50策略
            # 改为检查短期均线（5日）上穿长期均线（20日）的简化版本
            cross_ma_simple = False
            try:
                if len(daily_data) >= 20:
                    ...
                    for i in range(-3, 0):
                        if i >= -len(daily_data) + 1:
                            if (daily_data['ma5'].iloc[i] > daily_data['ma20'].iloc[i] and 
                                daily_data['ma5'].iloc[i-1] <= daily_data['ma20'].iloc[i-1]):
                                cross_ma_simple = True
                                break
            except Exception:
                cross_ma_simple = False
            ...
            weekly_data = self.base_op.get_weekly_data(stock_code, weeks=50)  # 减少到50周
            if weekly_data is not None and not weekly_data.empty and len(weekly_data) >= 40:
                ...
                if len(weekly_data) >= 40:
                    last40 = weekly_data['close'].tail(40)
                    prev20 = last40.iloc[:20]
                    recent20 = last40.iloc[20:]
                    
                    if not prev20.empty and not recent20.empty:
                        prev_mean = float(prev20.mean())
                        recent_mean = float(recent20.mean())
                        
                        if prev_mean > 0 and recent_mean > 0:
                            # 放宽：只要近期均值不高于前期均值太多
                            weekly_mean_down = recent_mean <= prev_mean * 1.05
            
            # 放宽条件3：只要满足其中一个策略即可
            if not (cross_ma_simple or weekly_mean_down):
                return None
            
            # 获取股票名称
            stock_name = self.base_op.get_stock_name(stock_code) or ""
            ...
            if cross_ma_simple and weekly_mean_down:
                confidence = 0.8
                reasons = 'cross_ma_simple+weekly_mean_down'
            elif cross_ma_simple:
                confidence = 0.7
                reasons = 'cross_ma_simple'
            else:
                confidence = 0.7
                reasons = 'weekly_mean_down'
            ...
            return {
                ...
                'reasons': reasons,
                'cross_ma_simple': cross_ma_simple,
    ...
    def _calculate_stock_score_relaxed(self, stock_row):
        """计算股票的预期得分（放宽版）"""
        # 基础得分
        ...
        # 策略得分
        strategy_score = 0
        if stock_row.get('cross_ma_simple', False):
            strategy_score += 5
        if stock_row.get('weekly_mean_down', False):
            strategy_score += 5
        
        return base_score + price_score + strategy_score
    
    def run_backtest(self):
        """运行修复版回测"""
        print("🚀 开始修复版GPU加速策略回测")
        print(f"📊 初始资金: {self.initial_capital:.2f}元")
        print(f"📈 策略组合: 放宽版 (weekly_mean_down 或 简化条件)")
        ...
        # 进度条
        pbar = tqdm(self.trading_dates, desc="修复版回测进度", unit="交易日")
        
        for current_date in pbar:
            # 更新进度条描述
            pbar.set_description(f"修复版回测 {current_date.date()}")
    ...
    def save_results(self, output_dir="fixed_gpu_backtest_results"):
        """保存回测结果"""
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='修复版GPU加速策略回测系统')
    ...
    parser.add_argument('--output-dir', type=str, default='fixed_gpu_backtest_results',
                       help='输出目录 (默认: fixed_gpu_backtest_results)')
    ...
    # 创建修复版回测系统
    backtest = FixedGPUBacktestSystem(
