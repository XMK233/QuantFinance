...
class StockAnalyzer:
    def __init__(self, data_dir):
        ...
        self.max_price = 20.0
        self.max_price_vs_ma20 = 0.03
        self.max_recent_range_position_20w = 0.55
        self.max_recent_range_position_52w = 0.55
    ...
    def load_stock_name_map(self):
        ...
        try:
            ...
            for _, row in df.iterrows():
                # 提取纯数字代码（去掉'sh.'或'sz.'前缀）
                code = row['code']
                if '.' in code:
                    pure_code = code.split('.')[1]
                    name_map[pure_code] = row['code_name']
                else:
                    name_map[code] = row['code_name']
            
            print(f"成功加载 {len(name_map)} 个股票名称映射")
            return name_map
            
        except Exception as e:
            print(f"加载股票名称映射时出错: {e}")
            return {}
    ...
    def load_stock_data(self, file_path):
        ...
        try:
            ...
            # 处理不同的文件名格式：
            # 1. "000001.csv" -> 股票代码 "000001"
            # 2. "sh.600000.csv" -> 股票代码 "600000"  
            # 3. "sz.000001.csv" -> 股票代码 "000001"
            
            if '.' in filename and filename.count('.') == 2:
                # 格式: sh.600000.csv 或 sz.000001.csv
                stock_code = filename.split('.')[1]
            else:
                # 格式: 000001.csv
            ...
            return df, {"code": stock_code, "name": stock_name}
    ...
    def calculate_features(self, df):
        ...
        lookback_52w = 52 if len(df) >= 52 else len(df)
        range_high_52w = df['high'].iloc[-lookback_52w:].max()
        range_low_52w = df['low'].iloc[-lookback_52w:].min()
        if range_high_52w > range_low_52w:
            features['price_in_range_52w'] = (current_close - range_low_52w) / (range_high_52w - range_low_52w)
        else:
            features['price_in_range_52w'] = 0.5
    ...
    def analyze_stock(self, file_path):
        """分析单只股票"""
        df, stock_info = self.load_stock_data(file_path)
        if df is None or stock_info is None:
            return None

        stock_name_upper = str(stock_info.get('name', '')).upper()
        if 'ST' in stock_name_upper:
            return None
        ...
        if features.get('price_vs_MA20', 0) > self.max_price_vs_ma20:
            return None

        pos_20w = features.get('price_in_range', 1.0)
        pos_52w = features.get('price_in_range_52w', 1.0)
        if pos_20w > self.max_recent_range_position_20w or pos_52w > self.max_recent_range_position_52w:
            return None
    ...
    def find_best_stocks(self, top_n=20):
        ...
        for i, file_path in enumerate(csv_files):
            ...
            if stock_data:
                if stock_data['stock_code'] in seen_codes:
                    continue
                seen_codes.add(stock_data['stock_code'])
    ...
    def generate_report(self, best_stocks):
        ...
        for i, stock in enumerate(best_stocks, 1):
            report += f"第{i}名: {stock['stock_name']} ({stock['stock_code']})\n"
...
def main():
    ...
    for i, stock in enumerate(best_stocks[:5], 1):
        print(f"{i}. {stock['stock_name']} ({stock['stock_code']}) - 评分: {stock['score']:.3f}")
    print("="*60)
