...
def update_daily_data(force_weekly: bool = False):
    """更新每日数据"""
    print("📊 开始更新每日数据...")
    ...
    try:
        ...
        # 更新日线数据
        print("📅 更新日线数据...")
        success, count, message = downloader.incremental_update(frequency='d', max_workers=6, stock_list=stock_list)
        ...
        if need_weekly:
            ...
            print("📈 更新周线数据...")
            success, count, message = downloader.incremental_update(frequency='w', max_workers=6, stock_list=stock_list)
            if success:
...
def main():
    ...
    # 更新数据
    update_daily_data(force_weekly=args.full)
